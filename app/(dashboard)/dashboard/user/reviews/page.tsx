...
import { Star, MessageSquare, Shield, Clock, Loader2, TrendingUp, Award, Users, Plus, Compass } from 'lucide-react';
      ...
      <div className="flex gap-3 items-center">
        ...
        <div>
          <h4 className="font-black text-slate-900 dark:text-white text-sm uppercase tracking-tight">
            {(review.reviewerId as any)?.name || 'Anonymous Traveler'}
          </h4>
...
const UserReviewsPage = () => {
  ...
  const { data: reviewsData, isLoading, isError } = useGetReviewsForUserQuery(user?._id || '');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Review Center</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1 font-semibold">Your expedition feedback and community reputation.</p>
        </div>
          ...
          <div className="flex gap-4">
            <div className="px-6 py-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 group hover:border-primary/30 transition-all">
              ...
              <div>
                ...
                <p className="text-xl font-black text-slate-900 dark:text-white leading-none">{(stats?.averageRating || 0).toFixed(1)}</p>
              </div>
            </div>
            <div className="px-6 py-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 group hover:border-primary/30 transition-all">
              ...
              <div>
                ...
                <p className="text-xl font-black text-slate-900 dark:text-white leading-none">{stats.totalReviews}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 dark:bg-slate-950 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
            ...
            <h3 className="text-xl font-black uppercase tracking-tight mb-2">Expedition Elite</h3>
            <p className="text-white/60 text-sm font-bold leading-relaxed mb-6">Your ratings help travelers trust your coordination skills. Maintain 4.5+ for elite status.</p>
            <div className="w-full bg-white/10 rounded-full h-1.5 mb-2 overflow-hidden">
               <div className="bg-primary h-full transition-all duration-1000" style={{ width: `${(stats?.averageRating || 0) * 20}%` }} />
            </div>
            <p className="text-[10px] font-black uppercase tracking-widest text-primary">Progress to Tier 1: 85%</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 p-8 space-y-6 shadow-sm">
             <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-3">
               <TrendingUp className="size-4 text-primary" strokeWidth={3} /> Trusted Stats
             </h4>
             <div className="space-y-4">
                {[
                  ...
                  <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl group hover:bg-primary/5 transition-all">
                    <div className="flex items-center gap-3">
                      <item.icon className="size-4 text-slate-400 group-hover:text-primary transition-colors" />
                      <span className="text-xs font-black uppercase tracking-widest text-slate-500 group-hover:text-slate-950 dark:group-hover:text-white">{item.label}</span>
                    </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
            <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/20">
              <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-3">
                 <Users className="size-5 text-primary" strokeWidth={3} /> Expedition Feedback
              </h3>
            ...
            <div className="p-8">
              {isLoading ? (
                <div className="py-20 flex flex-col items-center justify-center gap-4 text-slate-400">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">Synchronizing reviews...</p>
                </div>
              ) : isError ? (
                <div className="py-20 text-center text-rose-500">
                  <Star className="size-12 mx-auto mb-4 opacity-20" />
                  <p className="font-black uppercase tracking-tight">Error connecting to feedback logs</p>
                </div>
              ) : reviews.length > 0 ? (
                ...
                <div className="py-24 text-center">
                  ...
                  <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-tight">No Feedback Logs Yet</h4>
                  <p className="text-slate-400 text-sm font-bold max-w-xs mx-auto mt-2 italic">Finish an expedition to receive your first traveler badge.</p>
                </div>
              )}
            </div>
            
            <div className="p-8 bg-slate-50/50 dark:bg-slate-800/20 border-t border-slate-100 dark:border-slate-800 text-center">
               <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Trusted by the worldwide traveler community</p>
            </div>
          </div>
        </div>
