      ...
      <aside className="lg:col-span-4 flex flex-col gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 flex flex-col items-center text-center shadow-sm border border-slate-100 dark:border-slate-800">
          <div className="relative mb-6">
            <div className="size-32 rounded-full overflow-hidden border-4 border-primary bg-primary/10 flex items-center justify-center">
              {user?.image ? (
                ...
                <UserCircle className="size-20 text-primary" strokeWidth={1} />
              )}
            </div>
            <div className="absolute bottom-1 right-1 bg-primary size-8 rounded-full flex items-center justify-center border-4 border-white dark:border-slate-900 shadow-sm">
              <CheckCircle className="size-4 text-slate-900 fill-primary" strokeWidth={3} />
            </div>
          </div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white">{user?.name || 'Traveler'}</h1>
          ...
          <div className="grid grid-cols-3 w-full mt-8 border-t border-slate-100 dark:border-slate-800 pt-6">
            <div className="flex flex-col">
              <span className="text-lg font-black tracking-tight">{user?.visitedCountries?.length || '42'}</span>
              <span className="text-[10px] uppercase tracking-widest text-slate-400 font-black">Countries</span>
            </div>
            <div className="flex flex-col border-x border-slate-100 dark:border-slate-800">
              ...
              <span className="text-[10px] uppercase tracking-widest text-slate-400 font-black">Buddies</span>
            </div>
            <div className="flex flex-col">
              <span className="text-lg font-black tracking-tight">4.9</span>
              <span className="text-[10px] uppercase tracking-widest text-slate-400 font-black">Rating</span>
            </div>
          </div>
          <button className="w-full mt-8 bg-primary hover:bg-opacity-90 text-slate-900 font-black py-4 rounded-full flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
          <h3 className="text-lg font-black mb-4 flex items-center gap-2 text-slate-900 dark:text-white uppercase tracking-tight">
            <Target className="size-5 text-primary" strokeWidth={3} /> Travel Interests
          </h3>
          <div className="flex flex-wrap gap-2">
            {(user?.travelInterests?.length ? user.travelInterests : [
              "Solo Travel",
              "Hiking",
              "Street Photography",
              "Local Cuisine",
      ...
      <div className="lg:col-span-8 flex flex-col gap-8">
        <section>
          <div className="flex items-center justify-between mb-4 px-1">
            <h3 className="text-xl font-black flex items-center gap-2 text-slate-900 dark:text-white uppercase tracking-tight">
              <Calendar className="size-5 text-primary" strokeWidth={3} /> Upcoming Plans
            </h3>
          </div>
        ...
        <section>
          <h3 className="text-xl font-black mb-4 flex items-center gap-2 text-slate-900 dark:text-white uppercase tracking-tight px-1">
            <Globe className="size-5 text-primary" strokeWidth={3} /> Recently Visited
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-1">
            {[
              { country: "France", city: "Paris", img: "https://lh3.googleusercontent.com/aida-public/AB6AXuBIumrjc8oHIR8cO1JC3rnv_3jtcwd5yveCB4bDRXRo8VJCN3t2wZ6iWkVfegcBs-5pP1vvMnkQ2AdaBXb05j7WFKJEL96KYfjRyvRtZ5pfJylnURsBV_ZFFBg81PbG4KCjEr8yFFPimDNKrFBgwfo271OJ1fr0RQfKAGlJEgRoJ7OArVgecEvGuByOwgM3tqLpaN2OOsdtoAcg4fVrLnKD-Fg7OD7Js5ODW-laO-1KmX5bbU-IopW8k97jpcYZi-yN_XnJcWJ6-A" },
              { country: "Indonesia", city: "Bali", img: "https://lh3.googleusercontent.com/aida-public/AB6AXuAueTz_dARtS6902OuK9QQpK_lGbb3zIrfTILehiK8F1qrliZiKubw7MqxSHk4N532jwarcvCFtaSDMIV6N2mxiGyLtBmvoLUG7eU69UdaSVD9bNfiqog9r9uvDvPVDEdpCpsfMOOlUN_n0Fg-CwM3n47GADEh3VME3whNJAiBQ-kpEOp1WtT7hij56PoelY1P2grsUyccQTEyL6LrU6yv4rQ2dRWf6F-6Hi9dhVtfHEzkwKKTm9ZQU--f8posH_hE9H--2_HXxRA" },
              { country: "Morocco", city: "Marrakech", img: "https://lh3.googleusercontent.com/aida-public/AB6AXuBc0j0PM54YjRwX0Sgx1Qecdu2Dj0g5XmCQii5hyRXsC0V3gNCY6gSNSrYsNnWcXLtv-ZY71KEoSkrgbaa3gd1tZ-qq3ar8yEVKYZpQTKeermxqbt88N9veF4iOWcb_PuGlkrV4-vQl_wmJxPC4gi_v3mM5yYA8WeCbFsk8AytQyJUoHmg6u9fAIRjoT0Nuxc2RhM4LXLLG9RRLYWvwtBaz0VToI4qc55OW2gL_bK-phS1uF41kM8tivk6EbtqzXuaCUWOkzadhMA" },
        ...
        <section>
          <div className="flex items-center justify-between mb-4 px-1">
            <h3 className="text-xl font-black flex items-center gap-2 text-slate-900 dark:text-white uppercase tracking-tight">
              <Star className="size-5 text-primary" strokeWidth={3} /> Reviews (48)
            </h3>
            <div className="flex items-center gap-1 text-primary">
              {[1, 2, 3, 4, 5].map((i) => (
                <Star key={i} className={`size-3.5 fill-primary text-primary`} />
              ))}
              <span className="text-sm font-black ml-1 text-slate-900 dark:text-white font-mono">4.9</span>
            </div>
          </div>
            ...
            {[
              ...
              <div key={review.name} className="bg-white dark:bg-slate-900 rounded-3xl p-7 border border-slate-100 dark:border-slate-800 shadow-sm hover:border-primary/20 transition-all">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <h5 className="font-black text-slate-900 dark:text-white text-[15px]">{review.name}</h5>
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tight">{review.meta}</p>
                  </div>
