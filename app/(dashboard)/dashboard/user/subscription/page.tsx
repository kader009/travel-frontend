...
import { useInitializeSubscriptionMutation, useGetPaymentHistoryQuery } from '@/src/redux/store/api/endApi';

const SubscriptionPage = () => {
  const [initPayment, { isLoading: isInitializing }] = useInitializeSubscriptionMutation();
  const { data: historyData, isLoading: isHistoryLoading } = useGetPaymentHistoryQuery();
  ...
  const plans = [
    {
      id: 'basic',
      name: 'Essential Traveler',
      price: '10',
      icon: Zap,
      features: ['Up to 5 travel plans', 'Community access', 'Public profile', 'Email support'],
      color: 'blue'
    },
    {
      id: 'premium',
      ...
      price: '25',
      icon: Crown,
      features: ['Unlimited travel plans', 'Verified badge', 'Private groups', 'Priority support', 'Detailed insights'],
      color: 'primary',
      popular: true
    },
    {
      ...
      price: '60',
      icon: ShieldCheck,
      features: ['Everything in Explorer', 'Brand collaboration', 'Custom trip tags', 'Dedicated 24/7 concierge', 'Offline maps'],
      color: 'slate'
    }
  ];
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <section className="text-center space-y-4 max-w-2xl mx-auto">
        <h1 className="text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Choose Your Journey</h1>
        <p className="text-slate-500 dark:text-slate-400 font-semibold">Elevate your travel experience with our premium coordination tools and verified community status.</p>
      </section>
      ...
      <section className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-md overflow-hidden">
        <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
           <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight leading-none mb-1">Billing Legend</h2>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Your transaction history and logs</p>
              </div>
           </div>
           ...
           <div className="flex gap-4">
              <div className="px-5 py-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 flex items-center gap-4">
                 ...
                 <div>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-0.5">Total Spent</p>
                    <p className="text-sm font-black text-slate-900 dark:text-white leading-none">
                       ${historyData?.data?.reduce((acc, curr) => acc + curr.amount, 0) || 0}
                    </p>
                 </div>
              </div>
           </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/20">
                ...
                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Plan Details</th>
                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Amount</th>
                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Executed At</th>
              ...
              {isHistoryLoading ? (
                <tr>
                   <td colSpan={5} className="py-20 text-center">
                      ...
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Retrieving ledger...</p>
                   </td>
                </tr>
              ) : historyData?.data && historyData.data.length > 0 ? (
                historyData.data.map((tx) => (
                  <tr key={tx._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="px-10 py-6 font-mono text-[11px] text-slate-500 font-bold">{tx.transactionId}</td>
                    ...
                    <td className="px-10 py-6 text-center text-sm font-black text-slate-900 dark:text-white">${tx.amount}</td>
                    ...
                    <td className="px-10 py-6 text-right">
                       <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest inline-flex items-center gap-1.5 ${
                          tx.status === 'VALID' ? 'bg-emerald-500/10 text-emerald-500' :
                          tx.status === 'PENDING' ? 'bg-amber-500/10 text-amber-500' :
                          'bg-rose-500/10 text-rose-500'
                       }`}>
                          <div className={`size-1.5 rounded-full ${
                             tx.status === 'VALID' ? 'bg-emerald-500' :
                ...
                <tr>
                   <td colSpan={5} className="py-24 text-center">
                      ...
                      <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-tight">No Transactions Recorded</h4>
                      <p className="text-slate-400 text-sm font-bold mt-2">Activate a subscription to start your journey.</p>
                   </td>
                </tr>
              )}
            </tbody>
