...
const AdminTravelPlansPage = () => {
  ...
  const [planToDelete, setPlanToDelete] = useState<{ id: string, destination: string } | null>(null);
  ...
  const { data, isLoading, error: fetchError } = useGetAllTravelPlansAdminQuery(undefined);
  const [deleteTravelPlan, { isLoading: isDeleting }] = useDeleteTravelPlanAdminMutation();
  const [updateTravelPlan, { isLoading: isUpdating }] = useUpdateTravelPlanMutation();

  const travelPlans = (data?.data as ITravelPlan[]) || [];

  // Client-side filtering
  const filteredPlans = travelPlans.filter((plan: ITravelPlan) => 
  ...
  const handleDeleteClick = (plan: ITravelPlan) => {
    ...
    toast(
      <div className="flex flex-col gap-2 p-1">
        <p className="font-black text-slate-900 dark:text-white uppercase tracking-tighter text-sm">Delete Travel Plan?</p>
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await deleteTravelPlan(planToDelete.id).unwrap();
      if (res.success) {
        toast.success(`Plan for "${planToDelete.destination}" deleted successfully.`);
        setIsDeleteModalOpen(false);
        setPlanToDelete(null);
      }
    } catch (err: any) {
      toast.error(err?.data?.message || "An error occurred while deleting the plan.");
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateTravelPlan({
        id: editingPlan._id,
        data: editForm,
      }).unwrap();
      ...
      toast.error(err?.data?.message || "Failed to update travel plan.");
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 relative">
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Manage Travel Plans</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1 font-semibold">Monitor and manage all travel itineraries on the platform.</p>
        </div>
      </header>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <p className="font-black text-slate-900 dark:text-white uppercase tracking-tighter">Listed Plans</p>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-400">
            ...
            <p className="font-black uppercase tracking-widest text-xs">Fetching plans data...</p>
          </div>
        ) : fetchError ? (
          <div className="text-center py-16 text-rose-500">
            <Map className="size-12 mx-auto mb-4 opacity-40" />
            <p className="font-black uppercase tracking-tight">Error loading travel plans</p>
          </div>
        ) : filteredPlans.length === 0 ? (
          <div className="py-24 text-center">
            <Map className="size-12 mx-auto mb-4 text-slate-200 dark:text-slate-700" />
            <p className="font-black text-slate-400 uppercase tracking-widest text-xs">No plans found matching your criteria</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-800/50">
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Destination</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Creator</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Date Range</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Budget</th>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {filteredPlans.map((plan: ITravelPlan) => (
                  <tr key={plan._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="size-11 rounded-xl bg-primary/10 flex items-center justify-center overflow-hidden border border-slate-100 dark:border-slate-800 group-hover:border-primary/20 transition-all shrink-0">
                          {plan.images && plan.images[0] ? (
                            <img src={plan.images[0]} alt={plan.destination} className="size-full object-cover" />
                          ) : (
                            <Map className="size-5 text-primary opacity-60" />
                          )}
                        </div>
                        <div>
                          <p className="font-black text-slate-900 dark:text-white text-sm tracking-tight truncate max-w-[200px] capitalize">{plan.destination}</p>
                          <span className="text-[9px] font-black uppercase tracking-widest text-primary/70">{plan.travelType}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
                        ...
                        <span className="text-sm font-bold truncate max-w-[150px]">{plan.user?.name || 'Unknown'}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-1.5 text-slate-900 dark:text-slate-100">
                        ...
                        <span className="text-sm font-black tracking-tight">${plan.budget.min} - ${plan.budget.max}</span>
                      </div>
                    </td>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl relative z-10 border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-300">
            <div className="bg-slate-50 dark:bg-slate-800/50 px-8 py-6 flex justify-between items-center border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Edit Travel Plan</h3>
              </div>
              <button 
            ...
            <form onSubmit={handleUpdate} className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Travel Type</label>
                  <div className="relative">
                    <Plane className="absolute left-4 top-1/2 -translate-y-1/2 size-4 text-slate-400" />
                    <select 
                      value={editForm.travelType}
                      onChange={(e) => setEditForm({...editForm, travelType: e.target.value as TTravelType})}
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Min Budget ($)</label>
                  <input 
                    type="number" 
                    required
                    value={editForm.budget.min}
                    onChange={(e) => setEditForm({...editForm, budget: {...editForm.budget, min: Number(e.target.value)}})}
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Max Budget ($)</label>
                  <input 
                    type="number" 
                    required
                    value={editForm.budget.max}
                    onChange={(e) => setEditForm({...editForm, budget: {...editForm.budget, max: Number(e.target.value)}})}
                    className="w-full px-5 py-3 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700/50 rounded-2xl focus:ring-2 focus:ring-primary/20 outline-none font-bold text-slate-700 dark:text-slate-200 transition-all"
                  />
                </div>

                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Plan Description</label>
                  <textarea 
                    rows={4}
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 animate-in fade-in duration-300">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-8 shadow-2xl relative z-10 border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-300 text-center">
            ...
            <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight mb-2">Confirm Removal</h3>
            <p className="text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
              Are you absoluteley sure you want to remove the travel plan for <span className="text-rose-500 font-black underline">{planToDelete?.destination}</span>?
            </p>
            <div className="grid grid-cols-2 gap-4 mt-10">
              <button onClick={() => setIsDeleteModalOpen(false)} disabled={isDeleting} className="px-6 py-4 bg-slate-50 dark:bg-slate-800 text-slate-500 font-black rounded-2xl hover:bg-slate-100 text-xs uppercase cursor-pointer">Keep Plan</button>
