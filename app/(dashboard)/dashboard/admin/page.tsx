...
const AdminOverviewPage = () => {
  ...
  const { data: analyticsData, isLoading: isAnalyticsLoading } =
    useGetPaymentAnalyticsQuery(undefined);
  ...
  const stats = useMemo(() => {
    const users = Array.isArray(usersData?.data)
      ? (usersData?.data as IUser[])
      : [];
    const plans = Array.isArray(plansData?.data)
      ? (plansData?.data as ITravelPlan[])
      : [];
    ...
    const usersLast30 = users.filter(
      (user) => user.createdAt && new Date(user.createdAt) > thirtyDaysAgo,
    ).length;
    const usersPrev30 = users.filter(
      (user) =>
        user.createdAt &&
    ...
    const tripsLast30 = plans.filter(
      (plan) => plan.createdAt && new Date(plan.createdAt) > thirtyDaysAgo,
    ).length;
    const tripsPrev30 = plans.filter(
      (plan) =>
        plan.createdAt &&
        new Date(plan.createdAt) > sixtyDaysAgo &&
        new Date(plan.createdAt) <= thirtyDaysAgo,
    ).length;
    const tripGrowth =
      tripsPrev30 > 0
        ? ((tripsLast30 - tripsPrev30) / tripsPrev30) * 100
        : tripsLast30 > 0
          ? 100
          : 0;
    ...
    const revenueLast30 = payments
      .filter(
        (payment) =>
          payment.createdAt && new Date(payment.createdAt) > thirtyDaysAgo,
      )
    ...
    const revenuePrev30 = payments
      .filter(
        (payment) =>
          payment.createdAt &&
          new Date(payment.createdAt) > sixtyDaysAgo &&
    ...
    const completedPlans = plans.filter(
      (plan) => plan.status === 'ongoing' || plan.status === 'completed',
    ).length;
    const meetupSuccess =
      totalTrips > 0
        ? Math.min(100, Math.round((completedPlans / totalTrips) * 100))
        : 88;
    ...
    plans.forEach((plan) => {
      ...
      if (!destinationDataMap[destinationName]) {
        destinationDataMap[destinationName] = {
          count: 1,
          image: plan.images?.[0],
        };
      } else {
    ...
    const monthlyActivityData = monthNames.map((month) => ({
      month,
      trips: 0,
    }));
    plans.forEach((plan) => {
  ...
  return (
    <div className="animate-in fade-in duration-700 max-w-[1600px] mx-auto w-full">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1 font-semibold">
            Reviewing real-time growth and engagement metrics.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {/* Users Stat */}
        <div className="bg-white dark:bg-background-dark p-6 sm:p-8 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        <div className="lg:col-span-2 bg-white dark:bg-background-dark p-6 sm:p-8 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:shadow-xl hover:shadow-primary/5">
          <div className="flex items-center justify-between mb-12">
            <div>
              <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                <Activity className="size-5 text-primary" /> Trip Growth
                Analytics
              </h4>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">
                Platform Expansion Index
              </p>
            </div>
            <div className="flex gap-4">
              <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                <span className="size-2 rounded-full bg-primary/20 border border-primary"></span>{' '}
                Volume Log
              </span>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-background-dark p-6 sm:p-8 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col h-112">
          <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-tight mb-8">
            ...
            {stats.popularDestinations.length > 0 ? (
              stats.popularDestinations.map((destination, index) => (
                <div key={index} className="flex items-center gap-4">
                  <div className="size-12 rounded-xl bg-slate-100 dark:bg-background-dark shrink-0 overflow-hidden border border-slate-100 dark:border-slate-700 flex items-center justify-center relative">
                    {destination.image ? (
                      <Image
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="w-full bg-slate-100 dark:bg-background-dark h-2 rounded-full overflow-hidden">
                      <div
                        className={`${destination.color} h-full rounded-full transition-all duration-1000`}
                        style={{ width: destination.val }}
                      ></div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center justify-center h-full text-slate-300 font-black uppercase text-[10px] tracking-widest">
      ...
      <div className="bg-white dark:bg-background-dark rounded-4xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 sm:p-8 flex flex-col">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex -space-x-3">
            {stats.recentUsers.map((recentUser, index) => (
              <div
                key={index}
                className="size-9 rounded-full border-2 border-white dark:border-slate-900 bg-slate-200 dark:bg-background-dark overflow-hidden"
              >
                {recentUser.image ? (
                  <Image
                    src={recentUser.image}
                    alt="User"
                    width={36}
                    height={36}
                    className="size-full object-cover"
            ...
            {stats.totalUsers > 4 && (
              <div className="size-9 rounded-full border-2 border-white dark:border-slate-900 bg-primary text-slate-900 text-[10px] font-black flex items-center justify-center uppercase">
                +{stats.totalUsers - 4}
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 flex-1">
          {stats.recentUsers.map((userProfile, index) => (
            <div
              key={index}
              className="p-4 rounded-3xl border border-slate-100 dark:border-slate-800 flex items-center gap-4 hover:border-primary/20 transition-all group"
            >
              <div className="size-12 rounded-full bg-slate-50 dark:bg-background-dark overflow-hidden shrink-0 border border-slate-100 dark:border-slate-700 group-hover:scale-105 transition-transform">
                {userProfile.image ? (
                  <Image
