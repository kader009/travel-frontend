...
const PlanRequests = ({
  ...
  planName: string;
  planImage?: string;
  onRequestsFound: (found: boolean) => void;
}) => {
    ...
    <div className="space-y-4 max-w-2xl mx-auto px-4">
      ...
      <div className="space-y-3">
        {requests.map((req: IJoinRequest) => {
          const traveler = req.requester || req.user;
          return (
            <div
              key={req._id}
            ...
            >
              ...
              {req.message && (
                <p className="text-slate-500 dark:text-slate-400 text-xs font-bold line-clamp-2 italic border-l-2 border-slate-100 dark:border-slate-800 pl-4">
                  &quot;{req.message}
                </p>
              )}
...
const AdminJoinRequestsPage = () => {
  ...
  const [hasAnyReceivedRequests, setHasAnyReceivedRequests] = useState(false);
  ...
  const handleRequestsFound = useCallback((found: boolean) => {
    if (found) setHasAnyReceivedRequests(true);
  }, []);

  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700 px-4">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="w-full md:w-auto flex bg-slate-100 dark:bg-background-dark p-1.5 rounded-2xl">
          <button
            onClick={() => setActiveTab('received')}
            className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer ${
              activeTab === 'received'
            ...
            <>
                ...
                {myPlans.map((plan) => {
                  ...
                  return (
                    <PlanRequests
                      ...
                      onRequestsFound={handleRequestsFound}
                    />
                  );
                })}
              </div>
              {!hasAnyReceivedRequests && (
                <div className="py-24 text-center bg-slate-50 dark:bg-background-dark rounded-[3rem] border border-dashed border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-500">
                  ...
                  <p className="text-slate-500 font-bold mt-2">
                    No travelers have requested to join your current expeditions yet.
                  </p>
                </div>
              )}
            </>
